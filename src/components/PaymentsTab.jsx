...
export default function PaymentsTab({ bazarList, contributions, calculations }) {
  ...
  const getSettlementAction = () => {
    if (balance.Vaia > 0 && balance.Reazul < 0) {
      return {
        from: 'Reazul',
        to: 'Vaia',
        amount: Math.abs(balance.Reazul),
      };
    } else if (balance.Reazul > 0 && balance.Vaia < 0) {
      return {
        from: 'Vaia',
        to: 'Reazul',
        amount: Math.abs(balance.Vaia),
      };
    }
    return null;
  };
  ...
  const vaiaProgress = getProgressPercentage(contributions.Vaia, targetShare.Vaia);
  const reazulProgress = getProgressPercentage(contributions.Reazul, targetShare.Reazul);
    ...
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl glass-panel border border-slate-800/80 shadow-lg p-6 relative">
        ...
        <div className="mt-5 p-4 rounded-xl bg-slate-950/40 border border-slate-900/50 flex items-start space-x-3">
          ...
          <div className="text-xs text-slate-400 space-y-1">
            ...
            <p>1. Total Contribution Pool = ৳ {totalPool.toLocaleString()} (Vaia Paid: ৳ {contributions.Vaia.toLocaleString()} + Reazul Paid: ৳ {contributions.Reazul.toLocaleString()})</p>
            <p>2. Vaia Target Share (58.33%) = ৳ {Math.round(targetShare.Vaia).toLocaleString()}</p>
            <p>3. Reazul Target Share (41.67%) = ৳ {Math.round(targetShare.Reazul).toLocaleString()}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="p-5 rounded-2xl glass-panel border border-slate-800/80 shadow-md">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-2">
              <span className="text-sm font-semibold text-white">Vaia's Contribution Progress</span>
            </div>
            <span className="text-xs font-bold text-indigo-400">{vaiaProgress}%</span>
          </div>
          
          <div className="h-3 w-full bg-slate-950 rounded-full overflow-hidden border border-slate-900 p-0.5">
            <div 
              style={{ width: `${vaiaProgress}%` }}
              className="h-full rounded-full bg-gradient-to-r from-indigo-600 to-indigo-400 transition-all duration-500"
            />
          </div>

          <div className="flex items-center justify-between mt-4 text-xs text-slate-400">
            <span>Paid: ৳ {contributions.Vaia.toLocaleString()}</span>
            <span>Target: ৳ {Math.round(targetShare.Vaia).toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl glass-panel border border-slate-800/80 shadow-md">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-2">
              <span className="text-sm font-semibold text-white">Reazul's Contribution Progress</span>
            </div>
            <span className="text-xs font-bold text-emerald-400">{reazulProgress}%</span>
          </div>
          
          <div className="h-3 w-full bg-slate-950 rounded-full overflow-hidden border border-slate-900 p-0.5">
            <div 
              style={{ width: `${reazulProgress}%` }}
              className="h-full rounded-full bg-gradient-to-r from-emerald-600 to-emerald-400 transition-all duration-500"
            />
          </div>

          <div className="flex items-center justify-between mt-4 text-xs text-slate-400">
            <span>Paid: ৳ {contributions.Reazul.toLocaleString()}</span>
            <span>Target: ৳ {Math.round(targetShare.Reazul).toLocaleString()}</span>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-800/40">
              ...
              <tr className="hover:bg-slate-900/20 transition-colors">
                ...
                <td className="py-3.5 px-4 font-semibold text-slate-200">
                  <div>৳ {contributions.Vaia.toLocaleString()}</div>
                  <div className="text-[10px] text-slate-500 font-normal mt-0.5">
                    Base: ৳{(calculations.baseContributions?.Vaia || 0).toLocaleString()} | Bazar: ৳{(calculations.vaiaBazarSpent || 0).toLocaleString()}
                  </div>
                </td>
                <td className="py-3.5 px-4 text-slate-400">
                  ৳ {balance.Vaia > 0 ? balance.Vaia.toFixed(2) : '0.00'}
                </td>
                <td className="py-3.5 px-4 text-slate-300">৳ {targetShare.Vaia.toFixed(2)}</td>
                <td className="py-3.5 px-4 text-right font-bold">
                  <span className={balance.Vaia < 0 ? 'text-rose-400' : 'text-emerald-400'}>
                    {balance.Vaia < 0 ? `৳ ${Math.abs(balance.Vaia).toFixed(2)}` : '৳ 0.00'}
                  </span>
                </td>
              </tr>
              ...
              <tr className="hover:bg-slate-900/20 transition-colors">
                ...
                <td className="py-3.5 px-4 font-semibold text-slate-200">
                  <div>৳ {contributions.Reazul.toLocaleString()}</div>
                  <div className="text-[10px] text-slate-500 font-normal mt-0.5">
                    Base: ৳{(calculations.baseContributions?.Reazul || 0).toLocaleString()} | Bazar: ৳{(calculations.reazulBazarSpent || 0).toLocaleString()}
                  </div>
                </td>
                <td className="py-3.5 px-4 text-slate-400">
                  ৳ {balance.Reazul > 0 ? balance.Reazul.toFixed(2) : '0.00'}
                </td>
                <td className="py-3.5 px-4 text-slate-300">৳ {targetShare.Reazul.toFixed(2)}</td>
                <td className="py-3.5 px-4 text-right font-bold">
                  <span className={balance.Reazul < 0 ? 'text-rose-400' : 'text-emerald-400'}>
                    {balance.Reazul < 0 ? `৳ ${Math.abs(balance.Reazul).toFixed(2)}` : '৳ 0.00'}
                  </span>
                </td>
              </tr>
            </tbody>
